...
移动端优先的 AI 聊天应用，可用中转 API 或 Claude Code 通道（Pro/Max 订阅）调用 Claude 模型。支持添加到 iOS 主屏幕作为 PWA 全屏运行。

Cale 是 Quinn 的专属 AI 伙伴（男友设定）—— 除了聊天，他还能记住你、感知你的经期和心情、和你一起攒愿望清单、推荐歌和书，还能进入「小剧场」和你一起写长篇。
...
### 🎭 小剧场
- 独立的对话与历史列表（和普通聊天分开存储），共享同一套记忆库 / system prompt
- 小说质感长文：system prompt 自动追加长文写作指令，max_tokens 拉到上限
- 阅读式排版：无气泡、全宽、字号更大、行距更宽，像读小说

### 🔌 线路 & 订阅额度
- **线路选择**：中转 API / Claude Code 通道（用 Pro/Max 订阅额度，选此线路无需填地址和 Key）
  - Claude Code 通道当前为插桩：UI / 配置 / 额度已就绪，真实调用待在本地接入 `/api/claude-code`
- **订阅额度监控**：聊天顶部小圆点（绿/黄/红）指示状态，点开看 5 小时窗口与本周用量进度条（按 token 估算）

### 👋 欢迎页
- 首次打开 / 无对话时显示，极简排版：线条爱心、Cale & Quinn、where the wolf stays、Enter
